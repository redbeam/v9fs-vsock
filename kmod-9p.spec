...
%description
This package provides the 9P filesystem kernel module (9p.ko).
...
%{__make} %{?_smp_mflags} -C $kmod_kernel_dir M=%{_builddir}/v9fs-vsock-%{version}/$srcdir/fs/9p \
    CONFIG_9P_FS=m \
...
# Install the module
%{__make} -C $kmod_kernel_dir M=%{_builddir}/v9fs-vsock-%{version}/$srcdir/fs/9p \
